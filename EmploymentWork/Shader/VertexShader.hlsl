//プロパティ→HLSLコンパイラ→全般→シェーダーモデルを「Shader Model 5.1 (/5_1)」
//プロパティ→HLSLコンパイラ→出力ファイル→オブジェクトファイル名を「$(ProjectDir)%(Filename).vso」
...
struct VS_OUTPUT
{
    ...
    float4 P : POSITION1; // 座標( プロジェクション空間 )   //3Dから2Dに正規化された座標
    float4 VPosition : POSITION02; // 座標(ワールド空間*ビュー )
};
...
VS_OUTPUT main(VS_INPUT VSInput)
{
    ...
    //wを0にすることで平行移動成分を消している(平行移動自体させたくない)
    lLocalNormal.w = 0.0f;
