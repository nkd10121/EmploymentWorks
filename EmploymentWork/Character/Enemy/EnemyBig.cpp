...
namespace
{
	...
	/*モデル関係*/
	constexpr float kModelScale = 0.001f;		//モデルサイズ
...
void EnemyBig::Init()
{
	...
	//ステートパターンの初期化
	m_pState = std::make_shared<EnemyStateWalk>(std::dynamic_pointer_cast<EnemyBig>(shared_from_this()));
...
/// 索敵判定を生成する
/// </summary>
void EnemyBig::CreateSearchCollision()
{
	//当たり判定の作成
...
/// 索敵判定を削除する
/// </summary>
void EnemyBig::DeleteSearchCollision()
{
...
/// 索敵判定の半径を取得
/// </summary>
const float EnemyBig::GetSearchCollisionRadius() const
{
...
/// </summary>
void EnemyBig::OnTriggerEnter(const std::shared_ptr<Collide>& ownCol, const std::shared_ptr<Collidable>& send, const std::shared_ptr<Collide>& sendCol)
{
	//当たったオブジェクトのタグを取得する
...
/// </summary>
void EnemyBig::OnTriggerExit(const std::shared_ptr<Collide>& ownCol, const std::shared_ptr<Collidable>& send, const std::shared_ptr<Collide>& sendCol)
{
	//当たったオブジェクトのタグを取得する
	m_hitObjectTag = send->GetTag();
